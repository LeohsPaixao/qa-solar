...
- **Métodos**: `@DisplayName("Deveria [comportamento esperado]")`
- **Métodos de Teste**: `should[Comportamento]()` ou `shouldNot[Comportamento]()`
...
- **Explicit Waits**: `WebDriverWait` com condições específicas

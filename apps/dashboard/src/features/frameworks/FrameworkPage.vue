  ...
  <div class="framework-page">
    <div v-if="route.params.name" class="framework-detail">
      ...
      <div v-else-if="currentSummary" class="framework-content">
        <div class="summary-cards grid grid-cols-4">
          ...
          <SummaryCard v-if="currentSummary" title="Passed" :value="String(currentSummary.passed)"
            :label="`${currentSuccessRate}%`" variant="success" />
        ...
        <div v-if="currentSummary" class="charts-row grid grid-cols-2">
          <div class="chart-card card">
            <PassFailDonut :passed="currentSummary.passed" :failed="currentSummary.failed"
              :skipped="currentSummary.skipped" title="Distribuição de Resultados" />
          </div>
          <div class="chart-card card">
    ...
    <div v-else class="frameworks-list">
      ...
      <div v-else class="frameworks-grid grid grid-cols-3">
        <FrameworkCard v-for="(summary, framework) in byFramework" :key="framework" :framework="String(framework)"
          :summary="summary" :type="getFrameworkType(String(framework))" />
      </div>
    </div>
  </div>
</template>

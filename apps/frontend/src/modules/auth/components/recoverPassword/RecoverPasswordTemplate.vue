  ...
  <div class="recover-password-container main-content">
    <form data-testid="form-recover-password" class="form-recover-password" @submit.prevent="handleRecoverPassword">
      ...
      <div class="form-group-recover-password">
        ...
        <input data-testid="input-email-recover-password" type="email" id="email" autocomplete="username"
          v-model="email" class="input-email-recover-password" placeholder="Digite seu e-mail" required
          @blur="validateEmailField" :class="['input-email-recover-password', { 'input-error': errors.email }]" />
        <span data-testid="message-error-email-recover-password" v-if="errors.email" class="error-message">
          {{ errors.email }}
        </span>
      </div>
      <button data-testid="btn-recover-password" class="btn btn-recover btn-recover-password" type="submit"
    ...
    <p class="message-user-recover-password">
      <strong>Atenção:</strong> Este projeto não realiza envio de e-mails. Certifique-se de testar apenas para fins de
...
const handleRecoverPassword = (): void => {
  if (!errors.value.email) {
    sendForgotPasswordEmailMutation({ email: email.value.trim() }, {
